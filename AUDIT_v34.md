# AUDIT_v34 — Last-pass code audit of the five most submission-critical routines
...
(`code/v18_recompute.py` and `code/v18_sentinel_audit.py`).

**Audit date:** 2026-08-25 · **Seed frozen:** `20260819` · **N_BOOT:** `2000`

| # | Audit target | File : lines | Verdict |
|---|--------------|--------------|---------|
| 1 | Endpoint label builder | `v18_recompute.py` 100–180 (esp. 108–162) | ✅ Correct, documented |
| 2 | Missingness encoder (sentinel −3.0 → missing-aware) | `v18_sentinel_audit.py` 46–148 | ✅ Correct, sensitivity reported |
| 3 | Harmonic composite `H = 2(D+3)(Φ+3)/(D+Φ+6)` | `v18_recompute.py` 298–324 + 351–359 | ✅ Symbolically & numerically verified |
| 4 | Five-fold / nested CV (out-of-fold scores) | `v18_recompute.py` 525–545 | ✅ Seeded, leakage-free |
| 5 | Bootstrap / DeLong / community resampling | `v18_recompute.py` 416–519 + 555 | ✅ Seeded, traceable |

---

## 1. Endpoint label builder  (`v18_recompute.py` 100–180)
...
## 2. Missingness encoder — sentinel −3.0 → missing-aware rerun (`v18_sentinel_audit.py` 46–148)

**What it does.** The evidence matrix `X` is initialised to `−3.0` for every
missing cell (`v18_recompute.py:83`). `−3.0` is a *missing-data sentinel*, but the
...
## 3. Harmonic composite  `H = 2(D+3)(Φ+3)/(D+Φ+6)` (`v18_recompute.py` 298–324, 351–359)
...
## 4. Five-fold / nested CV — out-of-fold supervised scores (`v18_recompute.py` 525–545)
...
## 5. Bootstrap / DeLong / community resampling (`v18_recompute.py` 416–519, 555)
...
mutually consistent. The reproducibility harness (`verify_v34.py`) re-derives every
headline number and asserts it within tolerance.

**GO for submission** on the code-correctness axis (see `FINAL_v34_DELIVERY.md`).
