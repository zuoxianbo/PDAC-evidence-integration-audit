# STATS_v34 — Final statistical & methodological consistency check

**Purpose.** Final pre-submission verification that the v34 manuscript, the results
JSON (`v18_ncs_results.json`, `v18_sentinel_audit.json`, `v18_source_data.csv`), and
...
|-----------|--------|---------------|
| support-mean Φ (E3) = 0.889 → without-druggability 0.535 | `v18_sentinel_audit.json` S3 | `verify_v34.py` assertion |
| E3-A harmonic = 0.5752 = E1 harmonic (bit-identical) | `v18_sentinel_audit.json` S3 | confirms AUDIT §1 |
| E6 Random forest = 0.913 | `v18_source_data.csv` row 105 | `verify_v34.py` assertion |
| Fig.6 centrality 0.910 / harmonic 0.846 / tractability 0.780 | `v18_ncs_results.json` | `verify_v34.py` assertion |
| S2 quadrants a/b/c/d = 14.1 / 1.9 / 57.8 / 26.2 % | `v18_sentinel_audit.json` S1/S2 | figure provenance exact |
| All DeLong P < 10⁻¹⁶ | `v18_ncs_results.json` `delong_pairwise` | `verify_v34.py` assertion |
...
**GO for submission on the statistics/methodology axis** (see `FINAL_v34_DELIVERY.md`).
