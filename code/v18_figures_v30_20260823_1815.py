...
FIG = "/Users/zuoxianbo/Desktop/SCI论文/胰腺癌_submission/figures_v30_20260823_1815"
...
# =====================================================================
# Fig. 6 | pharmacological stress test + source-overlap sensitivity
# =====================================================================
def fig6():
    fig = plt.figure(figsize=(W2, 78 * MM))
    ...
    # (c) overlap-excluded sensitivity (honest gap)
    ax = fig.add_subplot(gs[0, 2])
    ax.set_xlim(0, 1); ax.set_ylim(0, 1); ax.axis("off")
    ax.add_patch(FancyBboxPatch((0.05, 0.24), 0.90, 0.62,
                 boxstyle="round,pad=0.02", fc="#F4F4F4", ec=C_NEU, lw=0.8))
    txt = ("SOURCE-OVERLAP SENSITIVITY — DOCUMENTED GAP\n\n"
           "E6 AUROC excluding tractability-overlap genes is not present in the "
           "regenerated v18 results (source raw inputs absent).\n\n"
           "Per the review plan this panel compares all-E6 vs "
           "tractability-overlap-excluded E6. No value is fabricated; render from "
           "recomputed source data when available (see Extended Data).")
    ax.text(0.5, 0.55, txt, ha="center", va="center", fontsize=5.2,
            color="#444444")
    ax.text(0.5, 0.92, "Overlap-excluded sensitivity", ha="center", fontsize=7,
            color=INK)
    panel(ax, "c")
    save(fig, "Fig6")
