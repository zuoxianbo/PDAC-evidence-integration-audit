...
recompute.py summarises the Dirichlet weight-space analysis (mean, s.d.,
...
recompute.py verbatim (data loading, endpoint construction, scorers) and
...
assert MARK in SRC, "section marker not found - recompute.py changed"
PREFIX = SRC.split(MARK)[0]

ns = {"__name__": "v18_prefix", "__file__": os.path.join(HERE, "recompute.py")}
exec(compile(PREFIX, "recompute.py[sections 1-4]", "exec"), ns)
...
p = os.path.join(RES, "weight_space.json")
