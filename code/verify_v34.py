...
# -*- coding: utf-8 -*-
"""
verify_v34.py — one-command reproducibility & integrity gate for the
...
2. Known headline constants re-derived from the frozen results artefacts:
     - E6 Random-forest AUROC = 0.9134  (v18_source_data.csv, row "E6 ... ,Random forest")
     - GDSC endpoint counts: n_pancreas_lines_with_ic50=29,
       n_drugs_sensitive_tertile=41, n_positive_genes=32   (v18_ncs_results.json)
...
Usage
-----
    python3 code/verify_v34.py            # verify (record manifest if absent)
    python3 code/verify_v34.py --strict  # fail if manifest absent (CI mode)
...
def main():
    ...
    recompute_src = open(os.path.join(HERE, "v18_recompute.py"),
                         encoding="utf-8").read()
    ...
    try:
        with open(os.path.join(RES, "v18_source_data.csv"), encoding="utf-8") as f:
        ...
        if hit:
            ...
            record("e6_rf_auroc", False, "row not found in v18_source_data.csv")
    except Exception as e:  # noqa
        record("e6_rf_auroc", False, f"exception: {e}")

    # ---- 2b. GDSC endpoint counts -------------------------------------
    try:
        g = json.load(open(os.path.join(RES, "v18_ncs_results.json"),
    ...
    try:
        s3 = json.load(open(os.path.join(RES, "v18_sentinel_audit.json"),
    ...
    fig_hashes = {}
    for i in range(1, 7):
        p = os.path.join(FIG, f"Fig{i}_v34.png")
        if os.path.exists(p):
            fig_hashes[f"Fig{i}_v34.png"] = sha256(p)
        else:
            record(f"Fig{i}_v34_present", False, "file missing")
    ...
    # ---- report -------------------------------------------------------
    print("=" * 64)
    print("verify_v34 — PDAC evidence-integration audit (NCS v34)")
    print("=" * 64)
