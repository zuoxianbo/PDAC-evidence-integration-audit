...
R = json.load(open(os.path.join(RES, "ncs_results.json")))
WS = json.load(open(os.path.join(RES, "weight_space.json")))
SENT = json.load(open(os.path.join(RES, "sentinel_audit.json")))

# ---- fixed cross-figure semantic palette (identical to figures.py) ----
...
def save(fig, name):
    fig.savefig(os.path.join(FIG, f"{name}.png"), dpi=400,
...
if __name__ == "__main__":
    print("Extended Data Figures 1-4 -> %s" % FIG)
    ok = True
