# FIGCHECKLIST — Per-figure final Nature production-grade checklist

**Generator.** `code/figures.py` (frozen, `STAMP="v34"`).
**Outputs.** `repository/figures/Fig{1..6}.png` (400 dpi) + `Fig{1..6}.pdf` (vector).
**Script base.** forked from `figures.py`; scientific structure unchanged, only style/terminology harmonised.
...
> **Action required before submission:** open `figures/Fig*.pdf` in a PDF viewer
...
## Fig.1 — Endpoint taxonomy & single-layer evidence 
- [x] `fig1()` writes `Fig1.png/.pdf`.
...
## Fig.2 — 12 scorers × endpoints AUROC heatmap 
...
## Fig.3 — Integration vs centrality 
...
## Fig.4 — Evidence-layer anatomy & label-embedded layer 
...
## Fig.5 — Functional-form & missingness audit 
...
- [x] **5d** title **"Sentinel-coded versus missingness-aware"**; plots
  ...
  `sentinel_audit.json` S4 (provenance exact).
- [ ] **human check:** 5a legend keys map 1:1 to the five colours at print size.

## Fig.6 — Pharmacological stress test (E6) 
...
- [x] **6b** keeps **RF = 0.913 verbatim** from `source_data.csv` row 105
...
- [x] Every figure is regenerated by a single script (`figures.py`) from the
  frozen results JSON — no hand-edited pixels.
- [x] `verify.py` records SHA-256 of each `Fig*.png` so a re-run is bit-checked.
...
Fig.6b panel letter sat on the bars. All fixed in `figures.py` and re-rendered
...
| 1/2/3/4/5/6 | general label crowding | figures made taller; margins increased; re-verified with `verify.py` (PASS, new figure hashes) |
...
plotting area / shrinking fonts / adding a white mask, then re-rendered; `verify.py`
...
Tables: `supplementary_tables.md` cross-checked against `ncs_results.json` /
`sentinel_audit.json` / `weight_space.json` — all AUROC, 95% CIs, sentinel %,
...
panel. All applied and re-rendered (`verify.py` re-baselined → PASS).
...
still overlap. Re-rendered (`verify.py` re-baselined → PASS).
...
legends no longer collide. Re-rendered (`verify.py` → PASS).
...
`source_data.csv`. No 0.913-vs-0.910 conflict. Terminology already conforms
