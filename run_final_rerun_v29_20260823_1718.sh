#!/usr/bin/env bash
# ===========================================================================
# run_final_rerun_v29_20260823_1718.sh
...
#        v18_recompute.py       -> results/v18_ncs_results.json,
#                                  results/v18_audit_report.json,
#                                  results/v18_source_data.csv
#        v18_weightspace.py     -> results/v18_weight_space.json
#        v18_sentinel_audit.py  -> results/v18_sentinel_audit.json
#        v18_figures.py         -> figures/Fig1..Fig6 (.pdf and .png)
#   3. Writes a run record (git commit SHA + results-manifest sha256) to
#      run_record_v29_20260823_1718.txt.
#
# HONESTY CAVEAT (IMPORTANT)
#   The raw third-party inputs are NOT redistributed in this repo (see
#   final_input_manifest_v29_20260823_1718.csv and the manifest's
...
#     code/v18_recompute.py hardcodes ROOT / OUT / GDSC_DIR to a different
#     author machine:
...
#     four path constants in v18_recompute.py to point at IN_DIR / OUT_DIR.
...
# Required raw inputs (sha256 / byte sizes in final_input_manifest_v29_20260823_1718.csv)
REQUIRED_INPUTS=(
...
echo "== PDAC audit final rerun (v29_20260823_1718) =="
...
if [[ $missing -eq 1 ]]; then
  ...
  echo "REPRODUCIBILITY_v29_20260823_1718.md for the full data-source list." >&2
  exit 1
fi
...
# ---------------------------------------------------------------------------
# 3) Run the pipeline, in order. (Assumes v18_recompute.py paths are resolved;
#    see the HARDCODED PATHS caveat above.)
# ---------------------------------------------------------------------------
python "$REPO_ROOT/code/v18_recompute.py"        # -> results/v18_ncs_results.json, v18_audit_report.json, v18_source_data.csv
python "$REPO_ROOT/code/v18_weightspace.py"      # -> results/v18_weight_space.json
python "$REPO_ROOT/code/v18_sentinel_audit.py"   # -> results/v18_sentinel_audit.json
python "$REPO_ROOT/code/v18_figures.py"          # -> figures/Fig1..Fig6 (.pdf and .png)

# ---------------------------------------------------------------------------
# 4) Write the run record: commit SHA + results-manifest sha256.
# ---------------------------------------------------------------------------
MANIFEST_HASH="$(cat \
  "$REPO_ROOT/results/v18_ncs_results.json" \
  "$REPO_ROOT/results/v18_audit_report.json" \
  "$REPO_ROOT/results/v18_source_data.csv" \
  "$REPO_ROOT/results/v18_sentinel_audit.json" \
  "$REPO_ROOT/results/v18_weight_space.json" \
  | shasum -a 256 | awk '{print $1}')"

RUN_END="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
cat > "$REPO_ROOT/run_record_v29_20260823_1718.txt" <<EOF
PDAC audit final rerun record (v29_20260823_1718)
...
echo "== Done. Record written to run_record_v29_20260823_1718.txt =="
