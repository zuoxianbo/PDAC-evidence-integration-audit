...
|---|---|---|---|
| P0-1 | E5/E6 source-overlap audit | Script ready, not runtime-run (raw inputs absent) | `code/audit_overlap_p0_1_v32_20260824.py`; E6 definition-overlap shown in Fig. 6; full recompute deferred to released inputs. |
| P0-2 | Leakage-safe nested cross-validation | Script ready, not runtime-run | `code/audit_nested_cv_p0_2_v32_20260824.py`; protocol described in Methods & Supp. Table 5. |
| P0-3 | Centrality annotation-density / study-bias | Script ready, not runtime-run | `code/audit_centrality_bias_p0_3_v32_20260824.py`; ED Fig. 1 drafted; caveat in Discussion. |
...
| P0-7 | Final clean rerun / provenance chain | One-command gate built (`code/verify_v34.py`); committed artefacts validated, raw-input full rerun pending released inputs | `verify_v34.py` asserts seed/sentinel/E3-A≡E1/E6-RF values and records SHA-256 of every figure + result artefact; no fabricated numbers. |
...
| Runtime status | protocol specified in `code/audit_nested_cv_p0_2_v32_20260824.py`; outer-fold results pending released inputs |
