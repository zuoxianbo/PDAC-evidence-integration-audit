...
All numbers are extracted from the precomputed result files (`ncs_results.json`, `weight_space.json`, `sentinel_audit.json`). No values are fabricated; items requiring the raw third-party inputs are flagged explicitly.
...
Each cell: AUROC (AUPRC). 95% percentile CIs from 2,000 stratified bootstrap resamples are in `source_data.csv`.
